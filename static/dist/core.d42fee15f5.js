...
async function pushAccountsToSub2api({ all = false } = {}) {
  ...
  if (all) {
    ...
    if (!confirm(`确认将选中的 ${ids.length} 个账号转移到 sub2api？\n\n导入成功的账号将从 grokcli-2api 本地删除；导入失败的账号会保留。`)) return;
    body = { account_ids: ids };
  }
  ...
  try {
    ...
    const fail = r && r.failed != null ? r.failed : 0;
    const total = r && r.total != null ? r.total : ok + fail;
    const cleaned = r && r.cleaned != null ? r.cleaned : 0;
    const cleanupFailed = r && r.cleanup_failed != null ? r.cleanup_failed : 0;
    const cleanupText = all ? "" : ` · 本地清理 ${cleaned}`;
    toast(`sub2api 导入完成：成功 ${ok} / 失败 ${fail} / 共 ${total}${cleanupText}`, fail === 0 && cleanupFailed === 0);
    if (!all && r && Array.isArray(r.cleaned_ids)) {
      for (const id of r.cleaned_ids) selectedAccountIds.delete(accountIdKey(id));
      try { await refreshAccountsListUI({ force: true }); } catch (_) {}
    }
    if ((fail || cleanupFailed) && r && Array.isArray(r.results)) {
      const firstErr = r.results.find((x) => x && !x.ok);
